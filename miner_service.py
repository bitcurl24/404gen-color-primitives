...
app = FastAPI(title="404gen-color-primitives")
state = State()
lock = threading.Lock()
...
def status(replacements_remaining: int = 0) -> JSONResponse:
    with lock:
        return JSONResponse({
            ...
            "payload": {"strategy": "color-primitives", "replacements_remaining": replacements_remaining},
        })


def _process_batch(req: GenerateRequest) -> None:
